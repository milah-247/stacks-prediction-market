...
(define-read-only (calculate-winnings (market-id uint) (user principal))
  ;; Optimized: early-exit if market not resolved or user has no winning bet
  (let (
    ...
    (if (not (get resolved market))
      ...
      (let (
        ...
        (user-bet (unwrap! (map-get? bets { market-id: market-id, user: user, outcome-index: winning-outcome }) (ok u0)))
        ...
        (if (is-eq user-amount u0)
          ...
          (let (
            ...
            (winning-pool (get pool (unwrap! (map-get? market-outcomes { market-id: market-id, outcome-index: winning-outcome }) (err u0))))
            (distributable-pool (- total-pool (/ (* total-pool PLATFORM-FEE-BPS) BPS-DENOMINATOR)))
          )
